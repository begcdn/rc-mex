...
class ProofStateSearchSmokeTests(unittest.TestCase):
    def test_smoke_runner_writes_requested_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Path-Family Beam Search Smoke Test", report)
            ...
            self.assertEqual(overlap["summary"]["diagnostic_future_mode"], "path_family_beam")
            overlap_report = (output / "error_overlap.md").read_text(encoding="utf-8")
            self.assertIn("Error Overlap Diagnostic", overlap_report)
            self.assertIn("path_family_beam Repeats Baseline Mistakes", overlap_report)
            ...
            self.assertIn("Path Family Beam Behavior Audit", behavior_report)
            ...
            self.assertIn("Path Family Beam Gold Survival Audit", survival_report)
            ...
            self.assertEqual(code_behavior["summary"]["diagnostic_mode"], "path_family_beam")
            self.assertIn("exact_formulas", code_behavior)
            self.assertIn("path_family_beam", code_behavior["exact_formulas"])
