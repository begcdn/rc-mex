...
class ProofStateSearchSmokeTests(unittest.TestCase):
    def test_smoke_runner_writes_requested_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Hybrid Relation-Proposal Proof-State Search Smoke Test", report)
            ...
            self.assertEqual(overlap["summary"]["diagnostic_future_mode"], "hybrid_relation_proposal_beam")
            overlap_report = (output / "error_overlap.md").read_text(encoding="utf-8")
            self.assertIn("Error Overlap Diagnostic", overlap_report)
            self.assertIn("hybrid_relation_proposal_beam Repeats Baseline Mistakes", overlap_report)
            ...
            self.assertIn("Hybrid Relation Proposal Behavior Audit", behavior_report)
            ...
            self.assertIn("Hybrid Relation Proposal Gold Survival Audit", survival_report)
            ...
            self.assertEqual(code_behavior["summary"]["diagnostic_mode"], "hybrid_relation_proposal_beam")
            self.assertIn("exact_formulas", code_behavior)
            self.assertIn("hybrid_relation_proposal_beam", code_behavior["exact_formulas"])
