...
class ProofStateSearchSmokeTests(unittest.TestCase):
    def test_smoke_runner_writes_requested_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("path_family_answer_ranker_v2_hits_at_1", metrics["metrics"])
            ...
            self.assertIn("path_family_answer_ranker_v2", metrics["scorer_constants"])
            ...
            self.assertIn("path_family_answer_ranker_v2", rows[0])
            ...
            self.assertTrue(rows[0]["path_family_answer_ranker_v2"]["candidate_answers"])
            self.assertIn("family_answer_ranker_diagnostics", rows[0]["path_family_answer_ranker_v2"])

            report = (output / "report.md").read_text(encoding="utf-8")
            self.assertIn("Path-Family Answer Ranker V2 Smoke Test", report)
            self.assertIn("Answer Ranker V2 Wins Over Path-Family", report)
            ...
            self.assertEqual(overlap["summary"]["diagnostic_future_mode"], "path_family_answer_ranker_v2")
            overlap_report = (output / "error_overlap.md").read_text(encoding="utf-8")
            self.assertIn("Error Overlap Diagnostic", overlap_report)
            self.assertIn("path_family_answer_ranker_v2 Repeats Baseline Mistakes", overlap_report)
            ...
            self.assertIn("Path Family Answer Ranker V2 Behavior Audit", behavior_report)
            ...
            self.assertIn("Path Family Answer Ranker V2 Gold Survival Audit", survival_report)
            ...
            self.assertEqual(code_behavior["summary"]["diagnostic_mode"], "path_family_answer_ranker_v2")
            self.assertIn("exact_formulas", code_behavior)
            self.assertIn("path_family_answer_ranker_v2", code_behavior["exact_formulas"])
            ...
            self.assertIn("Path-Family Answer Ranker V2 Hop Trace", trace)
