...
class ProofStateSearchSmokeTests(unittest.TestCase):
    def test_smoke_runner_writes_requested_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Two-Score Fixed Proof-State Search Smoke Test", report)
            self.assertIn("Two-Score Fixed Wins Over Current Proof-State", report)
            ...
            self.assertEqual(overlap["summary"]["diagnostic_future_mode"], "two_score_fixed_proof_state_beam")
            overlap_report = (output / "error_overlap.md").read_text(encoding="utf-8")
            self.assertIn("Error Overlap Diagnostic", overlap_report)
            self.assertIn("two_score_fixed_proof_state_beam Repeats Baseline Mistakes", overlap_report)
            ...
            self.assertIn("Two-Score Fixed Behavior Audit", behavior_report)
            ...
            self.assertIn("Two-Score Fixed Gold Survival Audit", survival_report)
            code_behavior = load_json(output / "code_behavior_audit.json")
            self.assertIn("summary", code_behavior)
            self.assertEqual(code_behavior["summary"]["diagnostic_mode"], "two_score_fixed_proof_state_beam")
            self.assertIn("exact_formulas", code_behavior)
            self.assertIn("two_score_fixed_proof_state_beam", code_behavior["exact_formulas"])
