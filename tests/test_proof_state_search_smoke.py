...
class ProofStateSearchSmokeTests(unittest.TestCase):
    def test_smoke_runner_writes_requested_outputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Two-Score Wide-Proposal Proof-State Search Smoke Test", report)
            self.assertIn("Two-Score Wide Wins Over Two-Score Fixed", report)
            ...
            self.assertEqual(overlap["summary"]["diagnostic_future_mode"], "two_score_wide_proposal_beam")
            overlap_report = (output / "error_overlap.md").read_text(encoding="utf-8")
            self.assertIn("Error Overlap Diagnostic", overlap_report)
            self.assertIn("two_score_wide_proposal_beam Repeats Baseline Mistakes", overlap_report)
            ...
            self.assertIn("Two-Score Wide Proposal Behavior Audit", behavior_report)
            ...
            self.assertIn("Two-Score Wide Proposal Gold Survival Audit", survival_report)
            ...
            self.assertEqual(code_behavior["summary"]["diagnostic_mode"], "two_score_wide_proposal_beam")
            self.assertIn("exact_formulas", code_behavior)
            self.assertIn("two_score_wide_proposal_beam", code_behavior["exact_formulas"])
