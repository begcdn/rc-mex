...
SPARQLPATH = os.environ.get("TOG_SPARQL_ENDPOINT", "http://xxx.xxx.xxx.xxx/sparql")  # depend on your own internal address and port, shown in Freebase folder's readme.md


def set_sparql_path(endpoint):
    global SPARQLPATH
    SPARQLPATH = endpoint
...
def execurte_sparql(sparql_txt):
    if SPARQLWrapper is None:
        raise RuntimeError("Missing dependency: install SPARQLWrapper before querying Freebase.")
