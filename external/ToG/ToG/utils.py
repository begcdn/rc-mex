...
def retrieve_top_docs(query, docs, model, width=3):
    ...
    if util is None:
        raise RuntimeError("Missing dependency: install sentence_transformers to use sentencebert pruning.")
...
def compute_bm25_similarity(query, corpus, width=3):
    ...
    if BM25Okapi is None:
        raise RuntimeError("Missing dependency: install rank_bm25 to use bm25 pruning.")
