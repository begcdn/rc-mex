...
TWO_SCORE_FIXED_CONSTANTS = {
    **TWO_SCORE_CONSTANTS,
    "retention_soft_loop_penalty": -0.08,
    "retention_soft_drift_penalty": -0.10,
    "retention_noisy_branch_penalty": -0.18,
    "obvious_start_return_penalty": -0.20,
    "obvious_repeated_entity_penalty": -0.18,
    "fixed_mode": 1.0,
}
...
HYBRID_RELATION_PROPOSAL_CONSTANTS = {
    **TWO_SCORE_FIXED_CONSTANTS,
    "relation_proposal_k": float(RELATION_PROPOSAL_K),
    "hybrid_relation_proposal_mode": 1.0,
    "alias_dictionary_size": 10.0,
}

PATH_FAMILY_CONSTANTS = {
    **HYBRID_RELATION_PROPOSAL_CONSTANTS,
    "answer_pool_cap": float(ANSWER_POOL_CAP),
    "path_family_mode": 1.0,
...
PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS = {
    **PATH_FAMILY_CONSTANTS,
    "answer_verifier_mode": 1.0,
    "verification_score_weight": 0.24,
    "specific_type_evidence_weight": 0.22,
    "remaining_need_evidence_weight": 0.14,
    "local_relation_evidence_weight": 0.10,
    "broad_only_penalty": -0.14,
    "contradiction_penalty": -0.26,
}

PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS = {
    **PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS,
    "answer_verifier_role_aware_mode": 1.0,
    "role_aware_specific_threshold": 0.35,
    "name_channel_weight": 0.35,
    "type_channel_weight": 0.65,
    "relation_channel_aux_weight": 0.40,
    "neighbor_channel_aux_weight": 0.20,
    "role_confusion_penalty": -0.18,
    "role_aware_verification_specific_weight": 0.65,
    "role_aware_verification_remaining_weight": 0.20,
...
PATH_FAMILY_LLM_RETENTION_CONSTANTS = {
    **PATH_FAMILY_CONCEPT_VERIFIER_CONSTANTS,
    "llm_retention_mode": 1.0,
    "llm_retention_union_cap": 2.0,
    "llm_retention_max_paths": 60.0,
}

PATH_FAMILY_WIDE_SYMBOLIC_CONSTANTS = {
    **PATH_FAMILY_CONCEPT_VERIFIER_CONSTANTS,
    "wide_symbolic_mode": 1.0,
...
PATH_FAMILY_ANY_HOP_ADJUDICATED_CONSTANTS = {
    **PATH_FAMILY_ANY_HOP_LLM_CONSTANTS,
    "final_adjudicator_mode": 1.0,
    "adjudicator_bonus": 0.45,
    "adjudicator_pool_size": 12.0,
}

RELATION_ALIASES = {
    "place of birth": ["born in", "birthplace", "was born", "native of"],
    "place of burial": ["buried in", "burial place", "grave location", "buried at"],
    "residence": ["lives in", "resides in", "resident of"],
    "author": ["wrote", "written by", "authored", "writer of"],
    "cast member": ["starred in", "acted in", "appears in"],
    "director": ["directed by", "director of", "made by"],
    "member of sports team": ["player for", "member of team", "plays for"],
    "headquarters location": ["headquartered in", "based in", "headquarters"],
    "legal form": ["business type", "company type", "legal status"],
    "occupation": ["job", "profession", "work as", "position"],
}

_SEMANTIC_RELATION_MODEL: Any | None = None
_SEMANTIC_RELATION_MODEL_ATTEMPTED = False
_SEMANTIC_EMBEDDING_CACHE: dict[str, Any] = {}
...
def main() -> None:
    ...
    for index, example in enumerate(examples, start=1):
        ...
        path_family_answer_verifier = run_path_family_answer_verifier(
            graph=graph,
            example=example,
            top_k=args.top_k,
            beam_width=args.beam_width,
        ...
        path_family_llm_retention = run_path_family_llm_retention(
            graph=graph,
            example=example,
            top_k=args.top_k,
            beam_width=args.beam_width,
        ...
        path_family_wide_symbolic = run_path_family_wide_symbolic(
            graph=graph,
            example=example,
            top_k=args.top_k,
            beam_width=args.beam_width,
        ...
        path_family_any_hop_adjudicated = run_path_family_any_hop_adjudicated(
            graph=graph,
            example=example,
            top_k=args.top_k,
            beam_width=args.beam_width,
    ...
    summary = {
        ...
        "scorer_constants": {
            ...
            "path_family_any_hop_adjudicated": PATH_FAMILY_ANY_HOP_ADJUDICATED_CONSTANTS,
    ...
    error_overlap = build_error_overlap(rows, future_key="path_family_answer_verifier", diagnostic_label="path_family_answer_verifier")
    ...
    gold_survival_audit = build_target_gold_survival_audit(
        ...
        rows,
        args,
        target_key="path_family_answer_verifier",
        target_label="Path Family Answer Verifier",
    )
    ...
    code_behavior_audit = build_two_score_code_behavior_audit(
        ...
        gold_survival_audit,
        target_key="path_family_answer_verifier",
        constants=PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS,
    )
    ...
    llm_retention_regression_audit = build_method_regression_audit(
        rows,
        baseline_key="path_family_concept_verifier",
        candidate_key="path_family_llm_retention",
        title="LLM Retention vs Concept Verifier Regression Audit",
        note="Same final answer ranking (concept verifier); only hop-level family retention changes to symbolic-union-LLM top picks.",
    )
    wide_symbolic_regression_audit = build_method_regression_audit(
        rows,
        baseline_key="path_family_wide_symbolic",
        candidate_key="path_family_llm_retention",
    ...
    adjudicator_value_audit = build_method_regression_audit(
        rows,
        baseline_key="path_family_any_hop_llm",
        candidate_key="path_family_any_hop_adjudicated",
        title="Final Adjudication vs Any-Hop LLM Regression Audit",
        note="Isolates micro-agent 3: one entity-aware listwise call over the top final candidates (skipped when the margin makes it a provable no-op).",
    )
    ...
    write_json(output_dir / "llm_retention_regression_audit.json", llm_retention_regression_audit)
    (output_dir / "llm_retention_regression_audit.md").write_text(
        write_method_regression_markdown(llm_retention_regression_audit), encoding="utf-8"
    )
    ...
    write_json(output_dir / "adjudicator_value_audit.json", adjudicator_value_audit)
    (output_dir / "adjudicator_value_audit.md").write_text(
        write_method_regression_markdown(adjudicator_value_audit), encoding="utf-8"
    )
    ...
    log_line("adjudicator_value_audit.md")
    if args.debug_trace:
...
def run_two_score_fixed_proof_state_beam(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
...
def run_hybrid_relation_proposal_beam(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
    ...
    for hop in [1, 2]:
        ...
        for state in states:
            ...
            for target_id, fragments in expansion_groups.items():
                ...
                signals["relation_proposal_k"] = float(RELATION_PROPOSAL_K)
                signals["hybrid_relation_proposal_mode"] = 1.0
                signals["semantic_relation_embeddings_used"] = 1.0 if semantic_used else 0.0
                relation_sequences = extend_relation_sequences(state.relation_sequences, steps)
                entity_sequences = extend_entity_sequences(graph, state.entity_sequences, steps)
                next_states.append(
                    SearchState(
        ...
        audit_trace.append(
            {
                ...
                "all_candidate_states": all_summaries,
                "selected_states": selected_summaries,
                "constants": HYBRID_RELATION_PROPOSAL_CONSTANTS,
                "relation_proposal_k": RELATION_PROPOSAL_K,
    ...
    result = search_result(
        ...
        example.gold_answer_ids,
        expansion_count,
        mode="hybrid_relation_proposal_beam",
        debug_trace=trace,
        answer_score_key="final_proof_score",
    )
    result["audit_trace"] = audit_trace
...
def run_path_family_beam(
    ...
    for hop in [1, 2]:
        ...
        for state in states:
            ...
            for target_id, fragments in expansion_groups.items():
                ...
                signals["relation_proposal_k"] = float(RELATION_PROPOSAL_K)
                signals["answer_pool_cap"] = float(ANSWER_POOL_CAP)
                signals["path_family_mode"] = 1.0
                signals["semantic_relation_embeddings_used"] = 1.0 if semantic_used else 0.0
                relation_sequences = extend_relation_sequences(state.relation_sequences, steps)
                entity_sequences = extend_entity_sequences(graph, state.entity_sequences, steps)
                next_states.append(
        ...
        family_records = build_path_family_records(next_states)
        llm_retention_diagnostics: dict[str, Any] = {}
        if wide_symbolic_beam:
            selected_records = family_records[: beam_width * int(PATH_FAMILY_WIDE_SYMBOLIC_CONSTANTS["wide_symbolic_beam_multiplier"])]
        elif llm_retention:
            selected_records, llm_retention_diagnostics = llm_union_family_selection(
                question=example.question,
                start_entity_name=example.start_entity_name,
                family_records=family_records,
                beam_width=beam_width,
                max_paths=int(PATH_FAMILY_LLM_RETENTION_CONSTANTS["llm_retention_max_paths"]),
            )
        else:
        ...
        all_summaries = [
            ...
            for rank, record in enumerate(family_records, start=1)
        ]
        selected_summaries = [
            summarize_path_family_record(record, rank)
        ...
        audit_trace.append(
            {
                ...
                "constants": path_family_ranker_constants(answer_ranker_v2, answer_verifier, answer_verifier_role_aware, concept_verifier, llm_retention, wide_symbolic_beam, any_hop_answers, final_adjudicator),
                "relation_proposal_k": RELATION_PROPOSAL_K,
                "answer_pool_cap": ANSWER_POOL_CAP,
                "semantic_relation_embeddings_used": semantic_used,
                "number_of_path_families_generated": len(family_records),
                "number_of_path_families_retained": len(selected_records),
                "average_answer_pool_size_per_family": avg(len(record["states"]) for record in family_records),
            }
        )
        if debug_trace:
            trace.append(
    ...
    result = path_family_search_result(
        ...
        debug_trace=trace,
        mode=path_family_ranker_mode(answer_ranker_v2, answer_verifier, answer_verifier_role_aware, concept_verifier, llm_retention, wide_symbolic_beam, any_hop_answers, final_adjudicator),
...
def run_path_family_answer_verifier(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
...
def run_path_family_answer_verifier_role_aware(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
    ...
    return run_path_family_beam(
        ...
        noisy_branch_threshold=noisy_branch_threshold,
        debug_trace=debug_trace,
        answer_verifier_role_aware=True,
    )
...
def run_path_family_llm_retention(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
...
def run_path_family_wide_symbolic(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
...
def apply_final_adjudication(
    ...
    Skips the call when the leader's margin exceeds the bonus (the call
    provably cannot change the answer). Abstention, error, or parse failure
    leaves the symbolic ranking untouched."""
    ...
    if margin > bonus:
        adjudication["skipped_provable_noop"] = True
        return
    top = candidates[:pool_size]
    blocks = []
    for candidate in top:
        readable = (candidate.get("paths") or [{}])[0].get("readable", "")[:180]
        blocks.append(f"{candidate['answer_label']} (path: {readable})")
    ...
    if response["error"] or response["pick"] is None:
        return
    chosen_id = str(top[response["pick"]]["answer_id"])
    adjudication["pick_label"] = str(top[response["pick"]]["answer_label"])
    for candidate in candidates:
        if str(candidate["answer_id"]) == chosen_id:
            candidate["score"] = float(candidate["score"]) + bonus
            candidate["adjudicator_bonus"] = bonus
    candidates.sort(key=lambda item: (-float(item["score"]), -float(item["best_path_score"]), item["answer_label"], item["answer_id"]))
    for rank, candidate in enumerate(candidates, start=1):
        candidate["rank"] = rank
    predicted_ids = {candidates[0]["answer_id"]}
    p = precision(gold_answer_ids, predicted_ids)
    r = recall(gold_answer_ids, predicted_ids)
    result.update(
        {
            "candidate_answers": candidates,
            "top_answer": candidates[0],
            "hits_at_1": bool(predicted_ids & gold_answer_ids),
            "exact_match": predicted_ids == gold_answer_ids if gold_answer_ids else not predicted_ids,
...
def run_path_family_any_hop_adjudicated(
    graph: KnowledgeGraph,
    example: SmokeExample,
    top_k: int,
    beam_width: int,
...
def path_family_ranker_mode(
    ...
    if final_adjudicator:
        return "path_family_any_hop_adjudicated"
...
def path_family_ranker_constants(
    ...
    if final_adjudicator:
        return PATH_FAMILY_ANY_HOP_ADJUDICATED_CONSTANTS
    if any_hop_answers and llm_retention:
    ...
    if wide_symbolic_beam:
        return PATH_FAMILY_WIDE_SYMBOLIC_CONSTANTS
    if concept_verifier:
        return PATH_FAMILY_CONCEPT_VERIFIER_CONSTANTS
    if answer_verifier_role_aware:
        return PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS
    if answer_verifier:
        return PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS
    if answer_ranker_v2:
        return PATH_FAMILY_ANSWER_RANKER_V2_CONSTANTS
    return PATH_FAMILY_CONSTANTS
...
def rank_relations_hybrid(question: str, frontier: list[Any]) -> list[dict[str, Any]]:
    ranked = []
    semantic_used = semantic_relation_model_available()
    for relation in frontier:
        label = f"{relation.predicate.replace('_', ' ')} {relation.direction}"
        lexical_similarity = char_ngram_similarity(question, label)
        frequency_bonus = 0.03 * min(1.0, float(relation.frequency) / 5.0)
        relation_texts = relation_text_expansions(str(relation.predicate), str(relation.direction))
        expansion_scores = [char_ngram_similarity(question, text) for text in relation_texts]
        expanded_lexical_score = max(expansion_scores, default=lexical_similarity)
        matched_aliases = [
            text for text, score in zip(relation_texts, expansion_scores)
            if text != label and score >= lexical_similarity + 0.02 and score >= 0.08
        ][:5]
        overlap = question_relation_term_overlap(question, relation_texts)
        semantic_score = semantic_relation_score(question, " ; ".join(relation_texts)) if semantic_used else 0.0
        non_semantic_score = max(lexical_similarity, expanded_lexical_score) + 0.05 * overlap + frequency_bonus
        if semantic_used:
            lexical_component = max(lexical_similarity, expanded_lexical_score) + 0.05 * overlap
            hybrid_relation_score = 0.55 * lexical_component + 0.35 * semantic_score + 0.10 * frequency_bonus
        else:
            hybrid_relation_score = non_semantic_score
        ranked.append(
            {
                "relation_id": relation.predicate,
                "direction": relation.direction,
                ...
                "frequency_bonus": frequency_bonus,
                "label_score": lexical_similarity + frequency_bonus,
                "expanded_lexical_score": expanded_lexical_score,
                "question_term_overlap": overlap,
                "semantic_score": semantic_score,
                "semantic_embeddings_used": semantic_used,
                "hybrid_relation_score": hybrid_relation_score,
                "proposal_score": hybrid_relation_score,
                "matched_aliases": matched_aliases,
                "relation_text": " ; ".join(relation_texts[:8]),
            }
        )
    ranked.sort(key=lambda item: (-item["hybrid_relation_score"], item["relation_id"], item["direction"]))
    return ranked


def relation_text_expansions(relation_id: str, direction: str) -> list[str]:
    label = relation_id.replace("_", " ")
    variants = {
        label,
        f"{label} {direction}",
        normalize_relation_variant(label),
    }
    for alias in RELATION_ALIASES.get(label.lower(), []):
        variants.add(alias)
        variants.add(f"{alias} {direction}")
    if direction == "backward":
        variants.add(f"inverse {label}")
        variants.add(f"{label} inverse")
    return sorted({variant.strip() for variant in variants if variant.strip()})


def normalize_relation_variant(label: str) -> str:
    text = label.lower().replace("-", " ").replace("_", " ")
    text = re.sub(r"\s+", " ", text).strip()
    replacements = {
        "place of birth": "birth place born in",
        "place of burial": "burial place buried in",
        "headquarters location": "headquarters based in",
        "cast member": "actor starred in",
        "member of sports team": "plays for team",
    }
    return replacements.get(text, text)


def question_relation_term_overlap(question: str, relation_texts: list[str]) -> float:
    question_terms = tokenize_content(question)
    relation_terms = set(token for text in relation_texts for token in tokenize_content(text))
    if not question_terms or not relation_terms:
        return 0.0
    return len(question_terms & relation_terms) / max(1, len(question_terms))


def semantic_relation_model_available() -> bool:
    return get_semantic_relation_model() is not None


def get_semantic_relation_model() -> Any | None:
    global _SEMANTIC_RELATION_MODEL, _SEMANTIC_RELATION_MODEL_ATTEMPTED
    if _SEMANTIC_RELATION_MODEL_ATTEMPTED:
...
def semantic_relation_score(question: str, relation_text: str) -> float:
    model = get_semantic_relation_model()
    if model is None:
        return 0.0
    try:
        import numpy as np  # type: ignore
        q_vec = semantic_embedding(question)
        r_vec = semantic_embedding(relation_text)
        denom = float(np.linalg.norm(q_vec) * np.linalg.norm(r_vec))
        if denom == 0.0:
            return 0.0
        return max(0.0, float(np.dot(q_vec, r_vec) / denom))
    except Exception:
        return 0.0


def semantic_embedding(text: str) -> Any:
    model = get_semantic_relation_model()
    if model is None:
        return []
    if text not in _SEMANTIC_EMBEDDING_CACHE:
        _SEMANTIC_EMBEDDING_CACHE[text] = model.encode(text, normalize_embeddings=False)
    return _SEMANTIC_EMBEDDING_CACHE[text]
...
def evidence_step(
    ...
    return {
        ...
        "proposal_score": candidate.get("proposal_score", candidate.get("label_score", 0.0)),
        "hybrid_relation_score": candidate.get("hybrid_relation_score", 0.0),
        "expanded_lexical_score": candidate.get("expanded_lexical_score", 0.0),
        "question_term_overlap": candidate.get("question_term_overlap", 0.0),
        "semantic_score": candidate.get("semantic_score", 0.0),
        "semantic_embeddings_used": candidate.get("semantic_embeddings_used", False),
...
def two_score_fixed_fragment_signals(
    graph: KnowledgeGraph,
    question: str,
    answer_type: str,
    state: SearchState,
    ...
    obvious_loop = two_score_obvious_retention_loop(state, steps, target_id, start_entity_ids, hop)
    non_loop_gate = 0.35 if obvious_loop else 1.0
    drift_signal = stronger_drift_penalty(
        graph=graph,
        question=question,
        answer_type=answer_type,
    ...
    # Retention is deliberately recall-friendly. Drift is logged and softly gated only for clear semantic drift.
    non_drift_gate = 0.75 if drift_signal < 0.0 else 1.0
    gated_future = raw_future * role_gate * non_drift_gate * non_loop_gate
    future_retention_bonus = min(gated_future, float(TWO_SCORE_FIXED_CONSTANTS["future_cap"]))
    answer_type_score = type_compatibility(graph, target_id, answer_type) if hop == 2 else 0.0
    soft_type_signal = float(TWO_SCORE_FIXED_CONSTANTS["retention_type_weight"]) * answer_type_score
    soft_progress = plausible_progress(question, graph, target_id, steps, hop)
    soft_diversity_signal = two_score_diversity_signal(state, steps)
    branch_size = max((int(step["branch_size"]) for step in steps), default=1)
    soft_loop_penalty = float(TWO_SCORE_FIXED_CONSTANTS["retention_soft_loop_penalty"]) if obvious_loop else 0.0
    soft_drift_penalty = float(TWO_SCORE_FIXED_CONSTANTS["retention_soft_drift_penalty"]) if (
        drift_signal < 0.0 and answer_type_score == 0.0 and current_relevance < 0.12
    ) else 0.0
    noisy_penalty = float(TWO_SCORE_FIXED_CONSTANTS["retention_noisy_branch_penalty"]) if branch_size >= noisy_branch_threshold else 0.0
    retention_delta = (
        float(TWO_SCORE_FIXED_CONSTANTS["retention_current_relevance_weight"]) * current_relevance
        + future_retention_bonus
        + soft_progress
        + soft_type_signal
        + soft_diversity_signal
    ...
    unresolved_need_penalty = float(TWO_SCORE_FIXED_CONSTANTS["unresolved_need_penalty_weight"]) * coverage["unresolved_need_score"] if hop == 2 else 0.0
    hard_loop_penalty = two_score_hard_loop_penalty(
        state=state,
        steps=steps,
        target_id=target_id,
    ...
    semantic_level_drift_penalty = float(TWO_SCORE_FIXED_CONSTANTS["semantic_level_drift_penalty"]) if (
        drift_signal < 0.0 and coverage["proof_role_coverage"] < 0.30 and coverage["answer_type_score"] == 0.0
    ) else 0.0
    surface_convergence_penalty_value = two_score_surface_convergence_penalty(
        state=state,
    ...
    redundancy_penalty = float(TWO_SCORE_FIXED_CONSTANTS["redundancy_penalty"]) if (
        repeats_relation_pattern(state, steps) and coverage["proof_role_coverage"] < 0.40
    ) else 0.0
    final_proof_delta = (
        float(TWO_SCORE_FIXED_CONSTANTS["final_role_coverage_weight"]) * coverage["proof_role_coverage"]
        + float(TWO_SCORE_FIXED_CONSTANTS["final_answer_type_weight"]) * coverage["answer_type_score"]
        + float(TWO_SCORE_FIXED_CONSTANTS["final_current_relevance_weight"]) * current_relevance
        + useful_convergence
        + unresolved_need_penalty
        + hard_loop_penalty
        + semantic_level_drift_penalty
...
def two_score_obvious_retention_loop(
    state: SearchState,
    steps: list[dict[str, Any]],
    target_id: str,
    start_entity_ids: set[str],
    ...
    if hop == 2 and target_id in start_entity_ids:
        return True
    evidence = state.evidence + steps
    if has_repeated_entity_cycle(evidence):
        return True
    return False
...
def merge_two_score_fixed_signal_dicts(previous: dict[str, float], current: dict[str, float]) -> dict[str, float]:
    merged = dict(previous)
    current_only_keys = {
        "role_gate",
        "non_drift_gate",
...
def summarize_two_score_state(state: SearchState, rank: int) -> dict[str, Any]:
    ...
    return {
        ...
        "proposal_score": last_step.get("proposal_score", 0.0),
        "hybrid_relation_score": last_step.get("hybrid_relation_score", 0.0),
        "expanded_lexical_score": last_step.get("expanded_lexical_score", 0.0),
...
def build_prediction_row(
    ...
    path_family_any_hop_adjudicated: dict[str, Any],
    ...
    path_family_any_hop_adjudicated_correct = path_family_any_hop_adjudicated["hits_at_1"]
    ...
    if baseline_correct and proof_correct and path_family_verifier_role_aware_correct:
    ...
    elif path_family_any_hop_adjudicated_correct and not path_family_any_hop_llm_correct:
        failure_type = "path_family_any_hop_adjudicated_correct"
        ...
        failure_type = "hybrid_relation_proposal_correct"
    ...
    elif not baseline["gold_generated"] and not proof_state["gold_generated"] and not path_family_answer_verifier_role_aware["gold_generated"]:
        failure_type = "gold_not_generated"
    elif not baseline["candidate_answers"] or not proof_state["candidate_answers"] or not path_family_answer_verifier_role_aware["candidate_answers"]:
    ...
    return {
        ...
        "path_family_any_hop_adjudicated": path_family_any_hop_adjudicated,
...
def compute_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    path_family_ah_adj = [row["path_family_any_hop_adjudicated"] for row in rows]
    ...
    path_family_verifier_overlap = [error_overlap_case(row, future_key="path_family_answer_verifier") for row in rows]
    path_family_verifier_role_aware_overlap = [error_overlap_case(row, future_key="path_family_answer_verifier_role_aware") for row in rows]
    return {
        ...
        "path_family_any_hop_adjudicated_hits_at_1": avg_bool(item["hits_at_1"] for item in path_family_ah_adj),
        ...
        "path_family_any_hop_adjudicated_exact_match": avg_bool(item["exact_match"] for item in path_family_ah_adj),
        ...
        "path_family_any_hop_adjudicated_final_answer_f1": avg(item["final_answer_f1"] for item in path_family_ah_adj),
        ...
        "path_family_any_hop_adjudicated_gold_generated_rate": avg_bool(item["gold_generated"] for item in path_family_ah_adj),
        ...
        "average_candidate_count_path_family_llm_retention": avg(item["candidate_count"] for item in path_family_llm_ret),
        "average_candidate_count_path_family_wide_symbolic": avg(item["candidate_count"] for item in path_family_wide_sym),
        ...
        "average_expansion_count_path_family_llm_retention": avg(item["expansion_count"] for item in path_family_llm_ret),
        "average_expansion_count_path_family_wide_symbolic": avg(item["expansion_count"] for item in path_family_wide_sym),
        ...
        "average_final_result_size_path_family_llm_retention": avg(item["final_result_size"] for item in path_family_llm_ret),
        "average_final_result_size_path_family_wide_symbolic": avg(item["final_result_size"] for item in path_family_wide_sym),
        ...
        "two_score_fixed_wins_over_baseline": sum(
            1 for row in rows
            if row["two_score_fixed_proof_state_beam"]["final_answer_f1"] > row["baseline_path_beam"]["final_answer_f1"]
        ),
        "baseline_wins_over_two_score_fixed": sum(
            1 for row in rows
            if row["baseline_path_beam"]["final_answer_f1"] > row["two_score_fixed_proof_state_beam"]["final_answer_f1"]
        ),
        "two_score_fixed_wins_over_current_proof_state": sum(
            1 for row in rows
            if row["two_score_fixed_proof_state_beam"]["final_answer_f1"] > row["soft_proof_state_beam"]["final_answer_f1"]
        ),
        "current_proof_state_wins_over_two_score_fixed": sum(
            1 for row in rows
            if row["soft_proof_state_beam"]["final_answer_f1"] > row["two_score_fixed_proof_state_beam"]["final_answer_f1"]
        ),
        "two_score_fixed_wins_over_two_score": sum(
            1 for row in rows
            if row["two_score_fixed_proof_state_beam"]["final_answer_f1"] > row["two_score_proof_state_beam"]["final_answer_f1"]
        ),
        "two_score_wins_over_two_score_fixed": sum(
            1 for row in rows
        ...
        "hybrid_wins_over_two_score_wide": sum(
            1 for row in rows
            if row["hybrid_relation_proposal_beam"]["final_answer_f1"] > row["two_score_wide_proposal_beam"]["final_answer_f1"]
        ),
        "two_score_wide_wins_over_hybrid": sum(
            1 for row in rows
            if row["two_score_wide_proposal_beam"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ),
        "hybrid_wins_over_baseline": sum(
            1 for row in rows
            if row["hybrid_relation_proposal_beam"]["final_answer_f1"] > row["baseline_path_beam"]["final_answer_f1"]
        ),
        "baseline_wins_over_hybrid": sum(
            1 for row in rows
            if row["baseline_path_beam"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ),
        "path_family_wins_over_hybrid": sum(
            1 for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ),
        "hybrid_wins_over_path_family": sum(
            1 for row in rows
            if row["hybrid_relation_proposal_beam"]["final_answer_f1"] > row["path_family_beam"]["final_answer_f1"]
        ),
        "path_family_wins_over_two_score_wide": sum(
            1 for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["two_score_wide_proposal_beam"]["final_answer_f1"]
        ),
        "two_score_wide_wins_over_path_family": sum(
            1 for row in rows
            if row["two_score_wide_proposal_beam"]["final_answer_f1"] > row["path_family_beam"]["final_answer_f1"]
        ),
        "path_family_wins_over_baseline": sum(
            1 for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["baseline_path_beam"]["final_answer_f1"]
        ),
        "baseline_wins_over_path_family": sum(
            1 for row in rows
            if row["baseline_path_beam"]["final_answer_f1"] > row["path_family_beam"]["final_answer_f1"]
        ),
        "path_family_answer_verifier_wins_over_path_family": sum(
            1 for row in rows
            if row["path_family_answer_verifier"]["final_answer_f1"] > row["path_family_beam"]["final_answer_f1"]
        ),
        "path_family_wins_over_answer_verifier": sum(
            1 for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["path_family_answer_verifier"]["final_answer_f1"]
        ),
        "path_family_answer_verifier_role_aware_wins_over_path_family": sum(
            1 for row in rows
            if row["path_family_answer_verifier_role_aware"]["final_answer_f1"] > row["path_family_beam"]["final_answer_f1"]
        ),
        "path_family_wins_over_answer_verifier_role_aware": sum(
            1 for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["path_family_answer_verifier_role_aware"]["final_answer_f1"]
        ),
        "path_family_answer_verifier_role_aware_wins_over_answer_verifier": sum(
            1 for row in rows
            if row["path_family_answer_verifier_role_aware"]["final_answer_f1"] > row["path_family_answer_verifier"]["final_answer_f1"]
        ),
        "path_family_answer_verifier_wins_over_role_aware": sum(
            1 for row in rows
            if row["path_family_answer_verifier"]["final_answer_f1"] > row["path_family_answer_verifier_role_aware"]["final_answer_f1"]
        ),
        "path_family_answer_verifier_wins_over_hybrid": sum(
            1 for row in rows
            if row["path_family_answer_verifier"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ),
        ...
        "llm_retention_wins_over_concept_verifier": sum(
            1 for row in rows
            if row["path_family_llm_retention"]["final_answer_f1"] > row["path_family_concept_verifier"]["final_answer_f1"]
        ),
        "concept_verifier_wins_over_llm_retention": sum(
            1 for row in rows
            if row["path_family_concept_verifier"]["final_answer_f1"] > row["path_family_llm_retention"]["final_answer_f1"]
        ),
        "llm_retention_wins_over_wide_symbolic": sum(
            1 for row in rows
            if row["path_family_llm_retention"]["final_answer_f1"] > row["path_family_wide_symbolic"]["final_answer_f1"]
        ),
        "wide_symbolic_wins_over_llm_retention": sum(
            1 for row in rows
            if row["path_family_wide_symbolic"]["final_answer_f1"] > row["path_family_llm_retention"]["final_answer_f1"]
        ),
        "adjudicated_wins_over_any_hop_llm": sum(
            1 for row in rows
            if row["path_family_any_hop_adjudicated"]["final_answer_f1"] > row["path_family_any_hop_llm"]["final_answer_f1"]
        ),
        "any_hop_llm_wins_over_adjudicated": sum(
            1 for row in rows
            if row["path_family_any_hop_llm"]["final_answer_f1"] > row["path_family_any_hop_adjudicated"]["final_answer_f1"]
        ),
        ...
        "llm_cost_per_question": {
            ...
            for method in ["path_family_llm_retention", "path_family_any_hop_llm", "path_family_any_hop_adjudicated"]
        },
        ...
        "both_correct": sum(
            ...
            and row["path_family_answer_verifier"]["hits_at_1"]
        ),
        "both_fail": sum(
            1 for row in rows
        ...
        "two_score_fixed_repeats_baseline_mistake": sum(
            1 for case in two_score_fixed_overlap
            if case["future_repeats_baseline_mistake"]
        ),
        "two_score_fixed_same_first_hop_as_baseline": sum(
            1 for case in two_score_fixed_overlap
            if case["same_first_hop_relation_as_baseline"]
        ),
        "two_score_fixed_same_drift_family_as_baseline": sum(
            1 for case in two_score_fixed_overlap
            if case["same_drift_family_as_baseline"]
        ),
        "two_score_fixed_gold_generated_but_ranked_low": sum(
            1 for row in rows
        ...
        "hybrid_repeats_baseline_mistake": sum(
            1 for case in hybrid_overlap
            if case["future_repeats_baseline_mistake"]
        ),
        "hybrid_same_first_hop_as_baseline": sum(
            1 for case in hybrid_overlap
            if case["same_first_hop_relation_as_baseline"]
        ),
        "hybrid_same_drift_family_as_baseline": sum(
            1 for case in hybrid_overlap
            if case["same_drift_family_as_baseline"]
        ),
        "hybrid_gold_generated_but_ranked_low": sum(
            1 for row in rows
            if row["hybrid_relation_proposal_beam"]["gold_generated"] and not row["hybrid_relation_proposal_beam"]["hits_at_1"]
        ),
        "path_family_repeats_baseline_mistake": sum(
            1 for case in path_family_overlap
            if case["future_repeats_baseline_mistake"]
        ),
        "path_family_same_first_hop_as_baseline": sum(
            1 for case in path_family_overlap
            if case["same_first_hop_relation_as_baseline"]
        ),
        "path_family_same_drift_family_as_baseline": sum(
            1 for case in path_family_overlap
            if case["same_drift_family_as_baseline"]
        ),
        "path_family_gold_generated_but_ranked_low": sum(
            1 for row in rows
            if row["path_family_beam"]["gold_generated"] and not row["path_family_beam"]["hits_at_1"]
        ),
        "path_family_answer_verifier_repeats_baseline_mistake": sum(
            1 for case in path_family_verifier_overlap
            if case["future_repeats_baseline_mistake"]
        ),
        "path_family_answer_verifier_same_first_hop_as_baseline": sum(
            1 for case in path_family_verifier_overlap
            if case["same_first_hop_relation_as_baseline"]
        ),
        "path_family_answer_verifier_same_drift_family_as_baseline": sum(
            1 for case in path_family_verifier_overlap
            if case["same_drift_family_as_baseline"]
        ),
        "path_family_answer_verifier_gold_generated_but_ranked_low": sum(
            1 for row in rows
            if row["path_family_answer_verifier"]["gold_generated"] and not row["path_family_answer_verifier"]["hits_at_1"]
        ),
        "path_family_answer_verifier_role_aware_repeats_baseline_mistake": sum(
            1 for case in path_family_verifier_role_aware_overlap
            if case["future_repeats_baseline_mistake"]
        ),
        "path_family_answer_verifier_role_aware_same_first_hop_as_baseline": sum(
            1 for case in path_family_verifier_role_aware_overlap
            if case["same_first_hop_relation_as_baseline"]
        ),
        "path_family_answer_verifier_role_aware_same_drift_family_as_baseline": sum(
            1 for case in path_family_verifier_role_aware_overlap
            if case["same_drift_family_as_baseline"]
        ),
        "path_family_answer_verifier_role_aware_gold_generated_but_ranked_low": sum(
            1 for row in rows
            if row["path_family_answer_verifier_role_aware"]["gold_generated"] and not row["path_family_answer_verifier_role_aware"]["hits_at_1"]
        ),
...
def write_report(metrics: dict[str, Any], rows: list[dict[str, Any]]) -> str:
    lines = [
        "# Path-Family Answer Verifier Smoke Test",
        "",
        ...
        "",
        "The main comparison is `path_family_beam` vs `path_family_answer_verifier` vs `path_family_answer_verifier_role_aware`. Relation proposal, path-family grouping, answer pool cap, family retention, relation proposal K, beam width, and search width stay unchanged; only final answer ranking changes.",
        "",
        "Path-family answer verifier constants:",
        "",
        "```json",
        json.dumps(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS, indent=2, sort_keys=True),
        "```",
        "",
        "Path-family answer verifier role-aware constants:",
        "",
        "```json",
        ...
        "```",
        "",
        "Path-family LLM retention constants:",
        "",
        "```json",
        json.dumps(PATH_FAMILY_LLM_RETENTION_CONSTANTS, indent=2, sort_keys=True),
        "```",
        "",
        "## Metrics",
        "",
        ...
        "```",
        "",
        "## Adjudicated Wins Over Any-Hop LLM",
        "",
        *debug_section_rows(select_debug_rows(rows, "adjudicated_over_any_hop_llm"), limit=5),
        "## Any-Hop LLM Wins Over Adjudicated",
        "",
        *debug_section_rows(select_debug_rows(rows, "any_hop_llm_over_adjudicated"), limit=5),
        ...
        *debug_section_rows(select_debug_rows(rows, "llm_retention_over_concept_verifier"), limit=5),
        "## Concept Verifier Wins Over LLM Retention",
        "",
        *debug_section_rows(select_debug_rows(rows, "concept_verifier_over_llm_retention"), limit=5),
        "## LLM Retention Wins Over Wide Symbolic Control",
        "",
        *debug_section_rows(select_debug_rows(rows, "llm_retention_over_wide_symbolic"), limit=5),
        "## Wide Symbolic Control Wins Over LLM Retention",
        "",
        *debug_section_rows(select_debug_rows(rows, "wide_symbolic_over_llm_retention"), limit=5),
        ...
        *debug_section_rows(select_debug_rows(rows, "path_family_verifier_over_path_family"), limit=5),
        "## Path-Family Wins Over Answer Verifier",
        "",
        *debug_section_rows(select_debug_rows(rows, "path_family_over_verifier"), limit=5),
        "## Answer Verifier Wins Over Hybrid Proposal",
        "",
        *debug_section_rows(select_debug_rows(rows, "path_family_verifier_over_hybrid"), limit=5),
        "## Path-Family Wins Over Hybrid Proposal",
        "",
        *debug_section_rows(select_debug_rows(rows, "path_family_over_hybrid"), limit=5),
        "## Hybrid Proposal Wins Over Path-Family",
        "",
        *debug_section_rows(select_debug_rows(rows, "hybrid_over_path_family"), limit=5),
        "## Path-Family Wins Over Wide Lexical Proposal",
        "",
        *debug_section_rows(select_debug_rows(rows, "path_family_over_two_score_wide"), limit=5),
        "## Wide Lexical Proposal Wins Over Path-Family",
        "",
        *debug_section_rows(select_debug_rows(rows, "two_score_wide_over_path_family"), limit=5),
        "## Hybrid Wins Over Wide Lexical Proposal",
        "",
        *debug_section_rows(select_debug_rows(rows, "hybrid_over_two_score_wide"), limit=5),
        "## Wide Lexical Proposal Wins Over Hybrid",
        "",
        *debug_section_rows(select_debug_rows(rows, "two_score_wide_over_hybrid"), limit=5),
        "## Hybrid Wins Over Baseline",
        "",
        *debug_section_rows(select_debug_rows(rows, "hybrid_over_baseline"), limit=5),
        "## Baseline Wins Over Hybrid",
        "",
        *debug_section_rows(select_debug_rows(rows, "baseline_over_hybrid"), limit=5),
        ...
        *debug_section_rows(select_debug_rows(rows, "two_score_fixed_over_two_score"), limit=5),
        "## Legacy Two-Score Wins Over Two-Score Fixed",
        "",
        *debug_section_rows(select_debug_rows(rows, "two_score_over_two_score_fixed"), limit=5),
        "",
        "## Proof-State Wins",
        "",
...
def select_trace_rows(rows: list[dict[str, Any]], debug_limit: int) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    for kind in [
        "hybrid_over_two_score_wide",
        "two_score_wide_over_hybrid",
        "path_family_over_hybrid",
        "hybrid_over_path_family",
        "adjudicated_over_any_hop_llm",
        "any_hop_llm_over_adjudicated",
        ...
        "wide_symbolic_over_llm_retention",
        ...
        "baseline_over_two_score_fixed",
        "two_score_fixed_over_two_score",
        "two_score_over_two_score_fixed",
        "future",
        "current_over_future",
        "surface_avoidance",
        "future_hurts",
...
def debug_trace_json_row(row: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "hybrid_relation_proposal_correct": row["hybrid_relation_proposal_beam"]["hits_at_1"],
        ...
        "hybrid_relation_proposal_top_answer": row["hybrid_relation_proposal_beam"]["top_answer"],
        "path_family_top_answer": row["path_family_beam"]["top_answer"],
        "path_family_answer_verifier_top_answer": row["path_family_answer_verifier"]["top_answer"],
        "path_family_answer_verifier_role_aware_top_answer": row["path_family_answer_verifier_role_aware"]["top_answer"],
        ...
        "hybrid_relation_proposal_debug_trace": row["hybrid_relation_proposal_beam"].get("debug_trace", []),
        ...
        "path_family_answer_verifier_constants": PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS,
        "path_family_answer_verifier_role_aware_constants": PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS,
        ...
        "path_family_answer_verifier_explanation": path_family_answer_verifier_choice_explanation(row),
        "path_family_answer_verifier_role_aware_explanation": path_family_answer_verifier_role_aware_choice_explanation(row),
    ...
    for row in rows:
        ...
        lines.extend(
            [
                ...
                "",
                f"Hybrid proposal top final state: {top_path_readable(hybrid_top)}",
                "",
                f"Path-family top final state: {top_path_readable(path_family_top)}",
                "",
                f"Path-family answer verifier top final state: {top_path_readable(path_family_verifier_top)}",
                "",
                ...
                f"Path-family answer verifier correct: `{path_family_verifier['hits_at_1']}`",
                ...
                *two_score_trace_lines(two_score_wide.get("debug_trace", [])),
                "### Hybrid Relation-Proposal Proof-State Hop Trace",
                "",
                *two_score_trace_lines(hybrid.get("debug_trace", [])),
                "### Path-Family Beam Hop Trace",
                "",
                *two_score_trace_lines(path_family.get("debug_trace", [])),
                "### Path-Family Answer Verifier Hop Trace",
                "",
                *two_score_trace_lines(path_family_verifier.get("debug_trace", [])),
                ...
                *two_score_wide_choice_explanation(row),
                "### Why Hybrid Proposal Chose This",
                "",
                *hybrid_relation_proposal_choice_explanation(row),
                "### Why Path-Family Beam Chose This",
                "",
                *path_family_choice_explanation(row),
                "### Why Path-Family Answer Verifier Chose This",
                "",
                *path_family_answer_verifier_choice_explanation(row),
...
def two_score_choice_explanation(row: dict[str, Any]) -> list[str]:
    return two_score_choice_explanation_for(
        row,
        key="two_score_proof_state_beam",
        label="two-score",
        compare_key="soft_proof_state_beam",
        compare_label="current proof-state",
    )


def two_score_fixed_choice_explanation(row: dict[str, Any]) -> list[str]:
    return two_score_choice_explanation_for(
        row,
        key="two_score_fixed_proof_state_beam",
        label="two-score fixed",
        compare_key="soft_proof_state_beam",
        compare_label="current proof-state",
    )
...
def hybrid_relation_proposal_choice_explanation(row: dict[str, Any]) -> list[str]:
    return two_score_choice_explanation_for(
        row,
        key="hybrid_relation_proposal_beam",
        label="hybrid relation proposal",
        compare_key="two_score_wide_proposal_beam",
        compare_label="wide lexical proposal",
    )


def path_family_choice_explanation(row: dict[str, Any]) -> list[str]:
    return two_score_choice_explanation_for(
        row,
        key="path_family_beam",
        label="path-family beam",
        compare_key="hybrid_relation_proposal_beam",
        compare_label="hybrid relation proposal",
    )


def path_family_answer_verifier_choice_explanation(row: dict[str, Any]) -> list[str]:
    lines = two_score_choice_explanation_for(
        row,
        key="path_family_answer_verifier",
        label="path-family answer verifier",
        compare_key="path_family_beam",
        compare_label="path-family beam",
    )
    top = row["path_family_answer_verifier"].get("top_answer") or {}
    components = top.get("answer_verifier", {}) if top else {}
    if components:
        lines.extend(
            [
                f"- Verification score: `{float(components.get('verification_score', 0.0)):.4f}`.",
                f"- Specific type evidence: `{float(components.get('specific_type_evidence', 0.0)):.4f}`.",
                f"- Remaining need evidence: `{float(components.get('remaining_need_evidence', 0.0)):.4f}`.",
                f"- Local relation evidence: `{float(components.get('local_relation_evidence', 0.0)):.4f}`.",
                f"- Broad-only penalty: `{float(components.get('broad_only_penalty', 0.0)):.4f}`.",
                f"- Contradiction penalty: `{float(components.get('contradiction_penalty', 0.0)):.4f}`.",
            ]
        )
    return lines


def path_family_answer_verifier_role_aware_choice_explanation(row: dict[str, Any]) -> list[str]:
    lines = two_score_choice_explanation_for(
        row,
        key="path_family_answer_verifier_role_aware",
        label="path-family answer verifier role-aware",
        compare_key="path_family_answer_verifier",
        compare_label="path-family answer verifier",
    )
    top = row["path_family_answer_verifier_role_aware"].get("top_answer") or {}
    components = top.get("answer_verifier", {}) if top else {}
    if components:
        lines.extend(
            [
                f"- Verification score: `{float(components.get('verification_score', 0.0)):.4f}`.",
                f"- Role-aware specific type score: `{float(components.get('role_aware_specific_type_score', 0.0)):.4f}`.",
                f"- Candidate name score: `{float(components.get('candidate_name_score', 0.0)):.4f}`.",
                f"- Candidate type score: `{float(components.get('candidate_type_score', 0.0)):.4f}`.",
                f"- Candidate relation label score: `{float(components.get('candidate_relation_label_score', 0.0)):.4f}`.",
                f"- Neighbor entity score: `{float(components.get('neighbor_entity_score', 0.0)):.4f}`.",
                f"- Role confusion penalty: `{float(components.get('role_confusion_penalty', 0.0)):.4f}`.",
                f"- Broad-only penalty: `{float(components.get('broad_only_penalty', 0.0)):.4f}`.",
                f"- Contradiction penalty: `{float(components.get('contradiction_penalty', 0.0)):.4f}`.",
                f"- Role-aware reason: {components.get('role_aware_reason', '')}.",
            ]
        )
    return lines
...
def two_score_choice_explanation_for(
    ...
    two_top = row[key].get("top_answer") or {}
    ...
    if row[key]["hits_at_1"] and not row[compare_key]["hits_at_1"]:
        lines.append(f"- Outcome: {label} beat {compare_label} on this question.")
    elif row[compare_key]["hits_at_1"] and not row[key]["hits_at_1"]:
        lines.append(f"- Outcome: {label} hurt a case {compare_label} got right.")
    elif row[key]["hits_at_1"] and row[compare_key]["hits_at_1"]:
...
def select_debug_rows(rows: list[dict[str, Any]], kind: str) -> list[dict[str, Any]]:
    if kind == "hybrid_over_two_score_wide":
        selected = [
            row for row in rows
            if row["hybrid_relation_proposal_beam"]["final_answer_f1"] > row["two_score_wide_proposal_beam"]["final_answer_f1"]
        ]
    elif kind == "path_family_over_hybrid":
        selected = [
            row for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ]
    elif kind == "adjudicated_over_any_hop_llm":
        selected = [
            row for row in rows
            if row["path_family_any_hop_adjudicated"]["final_answer_f1"] > row["path_family_any_hop_llm"]["final_answer_f1"]
        ]
    elif kind == "any_hop_llm_over_adjudicated":
        selected = [
            row for row in rows
            if row["path_family_any_hop_llm"]["final_answer_f1"] > row["path_family_any_hop_adjudicated"]["final_answer_f1"]
        ]
    ...
    elif kind == "concept_verifier_over_llm_retention":
        selected = [
            row for row in rows
            if row["path_family_concept_verifier"]["final_answer_f1"] > row["path_family_llm_retention"]["final_answer_f1"]
        ]
    elif kind == "llm_retention_over_wide_symbolic":
        selected = [
            row for row in rows
            if row["path_family_llm_retention"]["final_answer_f1"] > row["path_family_wide_symbolic"]["final_answer_f1"]
        ]
    elif kind == "wide_symbolic_over_llm_retention":
        selected = [
            row for row in rows
            if row["path_family_wide_symbolic"]["final_answer_f1"] > row["path_family_llm_retention"]["final_answer_f1"]
        ]
    ...
    elif kind == "path_family_over_verifier":
        selected = [
            row for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["path_family_answer_verifier"]["final_answer_f1"]
        ]
    elif kind == "path_family_verifier_over_hybrid":
        selected = [
            row for row in rows
            if row["path_family_answer_verifier"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ]
    ...
    elif kind == "path_family_over_two_score_wide":
        selected = [
            row for row in rows
            if row["path_family_beam"]["final_answer_f1"] > row["two_score_wide_proposal_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_wide_over_path_family":
        selected = [
            row for row in rows
            if row["two_score_wide_proposal_beam"]["final_answer_f1"] > row["path_family_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_wide_over_hybrid":
        selected = [
            row for row in rows
            if row["two_score_wide_proposal_beam"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ]
    elif kind == "hybrid_over_baseline":
        selected = [
            row for row in rows
            if row["hybrid_relation_proposal_beam"]["final_answer_f1"] > row["baseline_path_beam"]["final_answer_f1"]
        ]
    elif kind == "baseline_over_hybrid":
        selected = [
            row for row in rows
            if row["baseline_path_beam"]["final_answer_f1"] > row["hybrid_relation_proposal_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_wide_over_two_score_fixed":
        selected = [
            row for row in rows
            if row["two_score_wide_proposal_beam"]["final_answer_f1"] > row["two_score_fixed_proof_state_beam"]["final_answer_f1"]
        ]
    ...
    elif kind == "current_over_two_score_fixed":
        selected = [
            row for row in rows
            if row["soft_proof_state_beam"]["final_answer_f1"] > row["two_score_fixed_proof_state_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_fixed_over_baseline":
        selected = [
            row for row in rows
            if row["two_score_fixed_proof_state_beam"]["final_answer_f1"] > row["baseline_path_beam"]["final_answer_f1"]
        ]
    elif kind == "baseline_over_two_score_fixed":
        selected = [
            row for row in rows
            if row["baseline_path_beam"]["final_answer_f1"] > row["two_score_fixed_proof_state_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_fixed_over_two_score":
        selected = [
            row for row in rows
            if row["two_score_fixed_proof_state_beam"]["final_answer_f1"] > row["two_score_proof_state_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_over_two_score_fixed":
        selected = [
            row for row in rows
            if row["two_score_proof_state_beam"]["final_answer_f1"] > row["two_score_fixed_proof_state_beam"]["final_answer_f1"]
        ]
    elif kind == "two_score_over_current":
        selected = [
            row for row in rows
            if row["two_score_proof_state_beam"]["final_answer_f1"] > row["soft_proof_state_beam"]["final_answer_f1"]
        ]
...
def debug_section_rows(rows: list[dict[str, Any]], limit: int) -> list[str]:
    ...
    for row in rows[:limit]:
        ...
        path_family_verifier_top = row["path_family_answer_verifier"]["top_answer"] or {}
        ...
        path_family_verifier_path = top_path_readable(path_family_verifier_top)
        ...
        lines.extend(
            [
                ...
                f"- Hybrid proposal top answer: `{hybrid_top.get('answer_label', '')}`",
                ...
                f"- Path-family answer verifier top answer: `{path_family_verifier_top.get('answer_label', '')}`",
                f"- Path-family answer verifier evidence: {path_family_verifier_path}",
...
def build_error_overlap(
    ...
    elif future_key == "hybrid_relation_proposal_beam":
        summary["diagnostic_constants"] = HYBRID_RELATION_PROPOSAL_CONSTANTS
    elif future_key == "two_score_proof_state_beam":
        summary["diagnostic_constants"] = TWO_SCORE_CONSTANTS
    else:
        summary["diagnostic_constants"] = FUTURE_AWARE_V2_CONSTANTS
...
def build_target_gold_survival_audit(
    ...
    summary = {
        ...
        "gold_first_hop_missed_by_lexical_proposal": sum(
            1 for case in cases if not case.get("was_gold_relation_in_proposal_original_first_hop", False)
        ),
        "gold_first_hop_recovered_by_hybrid_proposal": sum(
            1 for case in cases
            if not case.get("was_gold_relation_in_proposal_original_first_hop", False)
            and case.get("was_gold_relation_in_proposal_hybrid_first_hop", False)
        ),
        "gold_second_hop_missed_by_lexical_proposal": sum(
            1 for case in cases if not case.get("was_gold_relation_in_proposal_original_second_hop", False)
        ),
        "gold_second_hop_recovered_by_hybrid_proposal": sum(
            1 for case in cases
            if not case.get("was_gold_relation_in_proposal_original_second_hop", False)
            and case.get("was_gold_relation_in_proposal_hybrid_second_hop", False)
        ),
        "hybrid_raised_gold_first_hop": sum(1 for case in cases if case.get("did_hybrid_raise_gold_first_hop", False)),
        "hybrid_raised_gold_second_hop": sum(1 for case in cases if case.get("did_hybrid_raise_gold_second_hop", False)),
    }
    return {"summary": summary, "cases": cases}
...
def build_target_gold_survival_case(
    ...
    relation_survival = gold_relation_survival_diagnostics(graph, row, args, proposal_k, hop1_all, hop1_selected, hop2_all, target_key)
...
def gold_relation_survival_diagnostics(
    ...
    first_rank_original = relation_rank_in_frontier(graph, row["question"], start_ids, first, args, ranker="original")
    first_rank_hybrid = relation_rank_in_frontier(graph, row["question"], start_ids, first, args, ranker="hybrid")
    first_targets, _ = graph.follow(start_ids, first.get("relation_id", ""), first.get("direction", ""), max_proofs=10000) if first else (set(), [])
    second_rank_original = relation_rank_in_frontier(graph, row["question"], first_targets, second, args, ranker="original")
    second_rank_hybrid = relation_rank_in_frontier(graph, row["question"], first_targets, second, args, ranker="hybrid")
    use_hybrid = target_key == "hybrid_relation_proposal_beam"
    first_rank = first_rank_hybrid if use_hybrid else first_rank_original
    second_rank = second_rank_hybrid if use_hybrid else second_rank_original
    ...
    return {
        ...
        "first_hop_relation_label_rank_original_lexical": first_rank_original,
        "first_hop_relation_label_rank_hybrid": first_rank_hybrid,
        "was_gold_first_hop_in_relation_proposal": bool(first_rank and first_rank <= proposal_k),
        "was_gold_relation_in_proposal_original_first_hop": bool(first_rank_original and first_rank_original <= proposal_k),
        "was_gold_relation_in_proposal_hybrid_first_hop": bool(first_rank_hybrid and first_rank_hybrid <= proposal_k),
        "did_hybrid_raise_gold_first_hop": rank_improved(first_rank_original, first_rank_hybrid),
        ...
        "second_hop_relation_label_rank_original_lexical": second_rank_original,
        "second_hop_relation_label_rank_hybrid": second_rank_hybrid,
        "was_gold_second_hop_in_relation_proposal": bool(second_rank and second_rank <= proposal_k),
        "was_gold_relation_in_proposal_original_second_hop": bool(second_rank_original and second_rank_original <= proposal_k),
        "was_gold_relation_in_proposal_hybrid_second_hop": bool(second_rank_hybrid and second_rank_hybrid <= proposal_k),
        "did_hybrid_raise_gold_second_hop": rank_improved(second_rank_original, second_rank_hybrid),
...
def relation_rank_in_frontier(
    ...
    ranked = rank_relations_hybrid(question, frontier) if ranker == "hybrid" else rank_relations(question, frontier)
...
def rank_improved(original_rank: int | None, hybrid_rank: int | None) -> bool:
    if hybrid_rank is None:
        return False
    if original_rank is None:
        return True
    return hybrid_rank < original_rank
...
def build_two_score_code_behavior_audit(
    rows: list[dict[str, Any]],
    gold_survival_audit: dict[str, Any],
    target_key: str = "two_score_proof_state_beam",
    constants: dict[str, float] | None = None,
) -> dict[str, Any]:
    constants = constants or TWO_SCORE_CONSTANTS
    states = collect_target_audit_states(rows, target_key)
    ...
    return {
        ...
        "scorer_locations": {
            **scorer_locations(),
            ...
            "two_score_fixed_proof_state_beam": {
                "file": "rc_mex/run_proof_state_search_smoke.py",
                "search_function": "run_two_score_fixed_proof_state_beam",
            ...
            "hybrid_relation_proposal_beam": {
                "file": "rc_mex/run_proof_state_search_smoke.py",
                "search_function": "run_hybrid_relation_proposal_beam",
                "relation_ranking_function": "rank_relations_hybrid",
                "scoring_function": "two_score_fixed_fragment_signals",
                "final_answer_ranker": "search_result(answer_score_key='final_proof_score')",
            },
        },
        ...
        "order_of_operations": {
            **order_of_operations_notes(),
            "two_score_specific": [
                "Local relation candidates are ranked before scoring. Fixed mode expands lexical ranked[:top_k]; wide-proposal mode expands lexical ranked[:RELATION_PROPOSAL_K]; hybrid mode expands hybrid ranked[:RELATION_PROPOSAL_K].",
                ...
                f"No hard first-hop diversity pruning is applied in {target_key}; hybrid proposal only changes pre-scoring relation proposal ranking.",
            ],
        },
        "gate_behavior": two_score_gate_behavior_notes(states),
        "scorer_constants": constants,
        "penalty_scale": {
            "average_positive_score_sum": avg(positive_values),
            "average_negative_penalty_sum": avg(negative_values),
            "average_penalty_to_positive_ratio": avg(ratio_values),
        ...
        "first_hop_diversity": {
            "implementation": f"No hard first-hop diversity pruning in {target_key}. Hybrid proposal keeps beam width fixed and only changes relation proposal ranking before scoring.",
...
def two_score_formula_notes() -> dict[str, dict[str, Any]]:
    ...
    formulas["two_score_fixed_proof_state_beam"] = {
        "retention_current_relevance": "current_relevance = max(step['relation_label_score'] for grouped steps)",
        "retention_raw_future": "raw_future = future_satisfiability(...)",
        "retention_gates": "role_gate is clamped to [0,1]; non_drift_gate = 0.75 only for soft semantic drift else 1.0; non_loop_gate = 0.35 only for obvious start-return/repeated-entity cycle else 1.0",
        "future_retention_bonus": "min(raw_future * role_gate * non_drift_gate * non_loop_gate, TWO_SCORE_FIXED_CONSTANTS['future_cap']); cap is applied after gating",
        "retention_score_delta": "0.50*current_relevance + future_retention_bonus + soft_progress + soft_type_signal + soft_diversity_signal + soft_loop_penalty + soft_drift_penalty + noisy_branch_penalty",
        "retention_penalties": "Retention uses soft penalties only: obvious-cycle loop penalty, clear low-relevance drift penalty, and noisy branch penalty.",
        "proof_role_coverage": "0.50*relation_role_coverage + 0.25*covered_need_score + 0.15*answer_type_score + 0.10*current step label score, capped at 1.0",
        "unresolved_need_penalty": "-0.25 * unresolved_need_score at hop 2",
        "final_proof_score_delta": "0.55*proof_role_coverage + 0.20*answer_type_compatibility + 0.25*current_relevance + useful_convergence + unresolved_need_penalty + hard_loop_penalty + semantic_level_drift_penalty + surface_convergence_penalty + redundancy_penalty",
    ...
    formulas["hybrid_relation_proposal_beam"] = {
        **formulas["two_score_fixed_proof_state_beam"],
        "relation_proposal": "At each hop, rank relations with hybrid_relation_score and expand ranked[:RELATION_PROPOSAL_K]. RELATION_PROPOSAL_K is hard-coded.",
        "hybrid_relation_score": "If embeddings unavailable: max(original lexical score, alias-expanded lexical score) + 0.05*question_term_overlap + frequency_bonus. If embeddings available: 0.55*lexical_component + 0.35*semantic_score + 0.10*frequency_bonus.",
        "proof_score_boundary": "The hybrid score is proposal-only. evidence_step preserves original label_score as relation_label_score for proof-state scoring.",
        "beam_retention": "After fixed two-score retention scoring, keep only beam_width states. Hybrid proposal does not increase final beam width.",
    }
    formulas["path_family_beam"] = {
        **formulas["hybrid_relation_proposal_beam"],
        "path_family_key": "Group generated bindings by (relation_sequence, direction_sequence, depth, coarse_relation_family_sequence). The final answer entity id/name is not part of the key.",
        "family_retention_score": "For each path family, score = max(binding retention_score) + 0.04*log1p(min(pool_size, ANSWER_POOL_CAP)) + branch_noise.",
        "family_beam_retention": "Keep top beam_width path families after scoring, then keep up to ANSWER_POOL_CAP concrete bindings inside each retained family.",
        "individual_sibling_pruning": "Sibling answer entities inside a retained relation path family do not compete against unrelated path families before final ranking.",
    ...
    formulas["path_family_answer_verifier"] = {
        **formulas["path_family_beam"],
        ...
        "verified_final_score": "old_path_family_final_score + 0.24*verification_score + 0.22*specific_type_evidence + 0.14*remaining_need_evidence + 0.10*local_relation_evidence + broad_only_penalty + contradiction_penalty + 0.03*log1p(num_paths)",
        "verification_score": "0.50*specific_type_evidence + 0.30*remaining_need_evidence + 0.20*local_relation_evidence, capped at 1.0.",
        "specific_type_evidence": "Question-specific condition match over candidate name and type labels, distinguishing film series/franchise, legal form, occupation, county/city/state/country, written work/book, school/institution, and similar specific types.",
        "remaining_need_evidence": "Overlap and char-ngram similarity between remaining question terms after relation evidence and candidate name/types/local relation labels.",
        "local_relation_evidence": "Condition match over the candidate entity's local outgoing/incoming relation labels.",
        "broad_only_penalty": "A graded -0.14 when the question asks for a specific class but the candidate only matches a broad answer type.",
        "contradiction_penalty": "A graded -0.26 when candidate-local evidence clearly contradicts the requested answer type. Unknown or missing evidence is not hard rejected.",
        "retention_boundary": "All verifier rewards and penalties are applied only after retained path-family pools are collected; family retention is unchanged.",
    }
    return formulas
...
def two_score_gate_behavior_notes(states: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "non_drift_gate": {
            "code_logic": "Legacy two-score uses 0.45 for active drift. Fixed two-score uses 0.75 for soft drift. Otherwise 1.0.",
            "possible_values_from_code": [0.45, 0.75, 1.0],
            "observed_accumulated_values": drift_gate_values,
        },
        "non_loop_gate": {
            "code_logic": "0.35 if loop signal is active. In fixed mode, this only fires for obvious start-entity return or exact repeated-entity cycle. Otherwise 1.0.",
        ...
        "combined_gate": {
            ...
            "note": "Fixed mode keeps gate debug fields current-only across hops, so observed products should also stay <= 1. Legacy mode may show accumulated debug fields.",
        },
    }
...
def scorer_locations() -> dict[str, dict[str, str]]:
    ...
    return {
        ...
        "path_family_answer_verifier": {
            "file": path,
            "search_function": "run_path_family_answer_verifier",
            "relation_ranker": "rank_relations_hybrid",
            "scoring_function": "two_score_fixed_fragment_signals",
            "family_builder": "build_path_family_records",
            "final_answer_ranker": "path_family_search_result(answer_verifier=True)",
            "answer_specific_scorer": "answer_side_verification",
        },
    }
...
def build_path_family_records(states: list[SearchState]) -> list[dict[str, Any]]:
    groups: dict[tuple[Any, ...], list[SearchState]] = defaultdict(list)
    for state in states:
        groups[path_family_key(state)].append(state)
    records = []
    for key, family_states in groups.items():
        family_states.sort(key=lambda item: (-float(item.soft_signals.get("final_proof_score", item.score)), -item.score, state_sort_key(item)))
        capped = family_states[:ANSWER_POOL_CAP]
        retention_score = max((state.score for state in capped), default=0.0)
        final_score = max((float(state.soft_signals.get("final_proof_score", 0.0)) for state in capped), default=0.0)
        pool_size = len(family_states)
        branch_noise = -0.05 * max(0.0, math.log1p(pool_size) - math.log1p(ANSWER_POOL_CAP / 4))
        score = retention_score + 0.04 * math.log1p(min(pool_size, ANSWER_POOL_CAP)) + branch_noise
        records.append(
            {
                "family_key": key,
                "states": capped,
                "all_pool_size": pool_size,
                "retention_score": retention_score,
                "final_proof_score": final_score,
                "family_score": score,
                "branch_noise": branch_noise,
            }
        )
    records.sort(key=lambda item: (-float(item["family_score"]), str(item["family_key"])))
    return records


def family_record_path_label(record: dict[str, Any]) -> str:
    states = record.get("states", [])
    if not states:
        return ""
    from rc_mex.micro_agents import relation_path_label

    return relation_path_label(states[0].evidence)


def llm_union_family_selection(
    question: str,
    start_entity_name: str,
    ...
    """Union beam: symbolic top-B plus the LLM's listwise top-B path families.

    The union can only add families to the symbolic selection, so gold
    retention is greater than or equal to the symbolic baseline by
    construction; any LLM error or parse failure degrades to exactly the
    symbolic top-B. The LLM sees relation labels and the question only —
    never entities from the pools, never gold."""
    from rc_mex.micro_agents import rank_relation_paths

    symbolic_records = family_records[:beam_width]
    candidates = family_records[:max_paths]
    labels: list[str] = []
    label_to_record: dict[str, dict[str, Any]] = {}
    for record in candidates:
        label = family_record_path_label(record)
        if label and label not in label_to_record:
            labels.append(label)
            label_to_record[label] = record
    ranked = rank_relation_paths(
        question=question,
        start_entity_name=start_entity_name,
        path_labels=labels,
        top_k=beam_width,
    )
    picked_records = [label_to_record[labels[index]] for index in ranked["picks"]]
    selected: list[dict[str, Any]] = []
    seen_keys: set[str] = set()
    for record in [*symbolic_records, *picked_records]:
        key = repr(record.get("family_key"))
        if key in seen_keys:
            continue
        seen_keys.add(key)
    ...
    diagnostics = {
        ...
        "union_selected": len(selected),
        "llm_raw_response": ranked.get("raw_response", ""),
    }
...
def path_family_key(state: SearchState) -> tuple[Any, ...]:
    ...
    family_sequence = tuple(relation_family_from_text(str(step.get("relation_id", ""))) for step in state.evidence)
    return relation_sequence, direction_sequence, len(relation_sequence), family_sequence


def flatten_family_states(records: list[dict[str, Any]], cap: int) -> list[SearchState]:
    out: list[SearchState] = []
    for record in records:
        out.extend(record.get("states", [])[:cap])
    return out


def summarize_path_family_record(record: dict[str, Any], rank: int) -> dict[str, Any]:
    states = list(record.get("states", []))
    top_state = states[0] if states else SearchState(frontier_ids=set(), score=0.0)
    summary = summarize_two_score_state(top_state, rank)
    summary.update(
        {
            "rank": rank,
            "family_key": repr(record.get("family_key")),
            "path_family_key": repr(record.get("family_key")),
            "number_of_bindings": int(record.get("all_pool_size", len(states))),
            "answer_pool_size": len(states),
            "retention_score": float(record.get("retention_score", 0.0)),
            "final_proof_score": float(record.get("final_proof_score", 0.0)),
            "family_score": float(record.get("family_score", 0.0)),
            "branch_noise": float(record.get("branch_noise", 0.0)),
            "pool_target_entities": [next(iter(state.frontier_ids), "") for state in states[:ANSWER_POOL_CAP]],
            "pool_target_labels": [state.evidence[-1].get("to_entity", "") for state in states[:10] if state.evidence],
            "states": [summarize_two_score_state(state, index) for index, state in enumerate(states[:10], start=1)],
        }
    )
    return summary
...
def answer_side_verification(
    ...
    local_relations = answer_local_relation_labels(graph, entity_id, limit=40)
    local_text = candidate_answer_evidence_text(graph, entity_id)
    snippets = candidate_evidence_snippets(graph, entity_id, condition_terms, local_relations)
    specific_type = condition_match_score(condition_terms, " ".join([graph.entity_name(entity_id), *candidate_types]).casefold())
    remaining = remaining_condition_score(question, graph, entity_id, evidence)
    relation_evidence = condition_match_score(condition_terms, " ".join(local_relations).casefold())
    broad_penalty = verifier_broad_only_penalty(graph, entity_id, question, answer_type, specific_type, local_text)
    ...
    verified_score = (
        ...
        + float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["specific_type_evidence_weight"]) * specific_type
        + float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["remaining_need_evidence_weight"]) * remaining
        + float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["local_relation_evidence_weight"]) * relation_evidence
        + broad_penalty
        + contradiction
        + support_bonus
        + concept_membership_bonus
    ...
    return {
        ...
        "verified_final_score": verified_score,
        "verification_score": verification,
        "specific_type_evidence": specific_type,
        "remaining_need_evidence": remaining,
        "local_relation_evidence": relation_evidence,
...
def answer_side_verification_role_aware(
    graph: KnowledgeGraph,
    question: str,
    answer_type: str,
    entity_id: str,
    ...
    condition_terms = specific_answer_condition_terms(question, answer_type)
    channels = candidate_verifier_evidence_channels(graph, entity_id)
    candidate_types = graph.entity_type_names(entity_id)
    name_score = condition_match_score(condition_terms, channels["candidate_name_text"])
    type_score = condition_match_score(condition_terms, channels["candidate_type_text"])
    relation_score = condition_match_score(condition_terms, channels["candidate_relation_label_text"])
    neighbor_score = condition_match_score(condition_terms, channels["neighbor_entity_text"])
    role_result = role_aware_specific_type_score(
        question=question,
        answer_type=answer_type,
        name_text=channels["candidate_name_text"],
        type_text=channels["candidate_type_text"],
        relation_text=channels["candidate_relation_label_text"],
        name_score=name_score,
        type_score=type_score,
    )
    specific_type = role_result["role_aware_specific_type_score"]
    remaining = remaining_condition_score_role_aware(question, graph, entity_id, evidence, channels)
    relation_evidence = min(
        1.0,
        float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["relation_channel_aux_weight"]) * relation_score
        + float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["neighbor_channel_aux_weight"]) * neighbor_score,
    )
    local_text = " ".join(
        [
            channels["candidate_name_text"],
            channels["candidate_type_text"],
            channels["candidate_relation_label_text"],
            channels["neighbor_entity_text"],
        ]
    )
    broad_penalty = verifier_broad_only_penalty_role_aware(
        graph=graph,
        entity_id=entity_id,
        question=question,
        answer_type=answer_type,
    ...
    contradiction = verifier_contradiction_penalty_role_aware(
        question=question,
        answer_type=answer_type,
        specific_type=specific_type,
        channels=channels,
    )
    role_confusion = role_result["role_confusion_penalty"]
    verification = min(
        1.0,
        float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_aware_verification_specific_weight"]) * specific_type
        + float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_aware_verification_remaining_weight"]) * remaining
        + float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_aware_verification_relation_weight"]) * relation_evidence,
    )
    support_bonus = 0.03 * math.log1p(float(support_count))
    verified_score = (
        old_score
    ...
    snippets = candidate_evidence_snippets_role_aware(graph, entity_id, condition_terms, channels)
    return {
        "old_path_family_final_score": old_score,
        "verified_final_score": verified_score,
        "verification_score": verification,
        ...
        "broad_only_penalty": broad_penalty,
        "contradiction_penalty": contradiction,
        "role_confusion_penalty": role_confusion,
        "support_bonus": support_bonus,
        "condition_terms": condition_terms,
        "candidate_types": candidate_types,
        "candidate_name_score": name_score,
        "candidate_type_score": type_score,
        "candidate_relation_label_score": relation_score,
        "neighbor_entity_score": neighbor_score,
        "role_aware_specific_type_score": specific_type,
        "role_aware_category": role_result["category"],
        "role_aware_reason": role_result["reason"],
        "candidate_name_text": channels["candidate_name_text"],
        "candidate_type_text": channels["candidate_type_text"],
        "candidate_relation_label_text": channels["candidate_relation_label_text"],
        "neighbor_entity_text": channels["neighbor_entity_text"],
        "local_relation_evidence_snippets": snippets,
        "evidence_snippets": snippets,
    }


def candidate_verifier_evidence_channels(graph: KnowledgeGraph, entity_id: str, limit: int = 40) -> dict[str, str]:
    relation_labels: list[str] = []
    neighbor_names: list[str] = []
    for relation in list(graph.iter_relations(entity_id))[:limit]:
        predicate = str(relation.get("predicate", "")).replace("_", " ")
        direction = str(relation.get("direction", ""))
        object_id = str(relation.get("object", ""))
        if predicate:
            relation_labels.append(f"{predicate} {direction}".strip())
        if object_id:
            neighbor_names.append(graph.entity_name(object_id))
    return {
        "candidate_name_text": graph.entity_name(entity_id).casefold(),
        "candidate_type_text": " ".join(graph.entity_type_names(entity_id)).casefold(),
        "candidate_relation_label_text": " ".join(relation_labels).casefold(),
        "neighbor_entity_text": " ".join(neighbor_names[:limit]).casefold(),
    }


def role_aware_specific_type_score(
    question: str,
    answer_type: str,
    name_text: str,
    type_text: str,
    ...
    del relation_text
    category = role_aware_answer_category(question, answer_type)
    identity_text = f"{name_text} {type_text}".casefold()
    base = min(
        1.0,
        float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["name_channel_weight"]) * name_score
        + float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["type_channel_weight"]) * type_score,
    )
    role_confusion = 0.0
    reason = "name/type lexical match"
    if category == "occupation":
        if contains_any(identity_text, ["occupation", "profession", "position", "job", "role", "field of work", "office"]):
            base = max(base, 0.85)
            reason = "candidate identity indicates an occupation/profession/position"
        if contains_any(identity_text, ["human", "person"]) and not question_asks_person(question):
            role_confusion = float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_confusion_penalty"])
            base = min(base, 0.08)
            reason = "candidate looks like a person, not the requested occupation"
    elif category == "film_series":
        if contains_any(identity_text, ["film series", "media franchise", "film franchise", "franchise", "series", "sequel", "prequel"]):
            base = max(base, 0.88)
            reason = "candidate identity indicates a film series/franchise"
        elif contains_any(identity_text, ["film", "movie", "television"]):
            base = min(base, 0.18)
            reason = "candidate is only a generic film/movie, not clearly a series/franchise"
    elif category == "legal_form":
        if contains_any(identity_text, ["legal form", "business type", "company type", "corporation type", "business entity", "juridical person"]):
            base = max(base, 0.85)
            reason = "candidate identity indicates a legal/business form"
        elif contains_any(identity_text, ["company", "organization", "enterprise"]):
            base = min(base, 0.18)
            reason = "candidate is only a company/organization, not a legal form"
    elif category in {"county", "city", "state", "country"}:
        level_terms = {
            "county": ["county"],
            "city": ["city", "town", "municipality", "big city"],
            "state": ["state", "province", "administrative territorial entity"],
            "country": ["country", "sovereign state", "unitary state"],
        }
        wrong_level_terms = {
            "county": ["country", "city", "town", "sovereign state", "unitary state"],
            "city": ["country", "county", "sovereign state", "unitary state"],
            "state": ["country", "city", "county", "sovereign state", "unitary state"],
            "country": ["city", "town", "county"],
        }
        if contains_any(identity_text, level_terms[category]):
            base = max(base, 0.88)
            reason = f"candidate identity indicates requested administrative level: {category}"
        elif contains_any(identity_text, wrong_level_terms[category]):
            role_confusion = float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_confusion_penalty"])
            base = min(base, 0.10)
            reason = f"candidate identity indicates the wrong administrative level for {category}"
    elif category == "written_work":
        if contains_any(identity_text, ["written work", "book", "novel", "literary work", "publication"]):
            base = max(base, 0.86)
            reason = "candidate identity indicates a written work/book/publication"
        elif contains_any(identity_text, ["human", "person", "city", "country", "event", "war"]):
            role_confusion = float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_confusion_penalty"])
            base = min(base, 0.08)
            reason = "candidate identity contradicts written-work answer role"
    return {
        "role_aware_specific_type_score": base,
        "role_confusion_penalty": role_confusion,
        "category": category,
        "reason": reason,
    }
...
def role_aware_answer_category(question: str, answer_type: str) -> str:
    q = question.casefold()
    if any(term in q for term in ["film series", "movie series", "series of films", "franchise"]):
        return "film_series"
    if any(term in q for term in ["occupation", "profession", "position"]):
        return "occupation"
    if any(term in q for term in ["legal form", "company type", "business type"]):
        return "legal_form"
    if "county" in q:
        return "county"
    if any(term in q for term in ["city", "town"]):
        return "city"
    if any(term in q for term in ["state", "province"]):
        return "state"
    if any(term in q for term in ["country", "sovereign state"]):
        return "country"
    if any(term in q for term in ["written work", "book", "novel", "publication"]):
        return "written_work"
    return answer_type or ""


def question_asks_person(question: str) -> bool:
    q = question.casefold()
    return q.startswith("who") or "which person" in q or "what person" in q
...
def remaining_condition_score_role_aware(
    question: str,
    graph: KnowledgeGraph,
    entity_id: str,
    evidence: list[dict[str, Any]],
    ...
    if not remaining_terms:
        return 0.0
    identity_text = f"{channels['candidate_name_text']} {channels['candidate_type_text']}"
    auxiliary_text = f"{channels['candidate_relation_label_text']} {channels['neighbor_entity_text']}"
    identity_overlap = sum(1 for term in remaining_terms if term in identity_text) / max(1, len(remaining_terms))
    auxiliary_overlap = sum(1 for term in remaining_terms if term in auxiliary_text) / max(1, len(remaining_terms))
    identity_soft = char_ngram_similarity(" ".join(sorted(remaining_terms)), identity_text)
    auxiliary_soft = char_ngram_similarity(" ".join(sorted(remaining_terms)), auxiliary_text)
    identity_score = min(1.0, 0.70 * identity_overlap + 0.30 * identity_soft)
    auxiliary_score = min(1.0, 0.70 * auxiliary_overlap + 0.30 * auxiliary_soft)
    del graph, entity_id
    return min(1.0, 0.80 * identity_score + 0.20 * auxiliary_score)


def verifier_broad_only_penalty_role_aware(
    graph: KnowledgeGraph,
    entity_id: str,
    question: str,
    answer_type: str,
    ...
    if specific_type >= 0.55:
        return 0.0
    q = question.casefold()
    category = role_aware_answer_category(question, answer_type)
    identity_text = f"{channels['candidate_name_text']} {channels['candidate_type_text']}"
    broad_score = type_compatibility(graph, entity_id, answer_type)
    if category == "film_series" and contains_any(identity_text, ["film", "movie", "television"]):
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"])
    if category == "legal_form" and contains_any(identity_text, ["company", "organization", "enterprise"]):
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"])
    if category in {"county", "city", "state", "country"} and broad_score > 0.0:
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"]) * 0.65
    asks_specific = any(
        term in q
        for term in [
            "film series", "franchise", "legal form", "company type", "business type",
            "occupation", "profession", "position", "county", "city", "state", "province",
            "country", "written work", "book", "novel", "university", "school",
        ]
    )
    if asks_specific and broad_score > 0.0:
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"])
    if asks_specific and not identity_text.strip():
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"]) * 0.35
    return 0.0


def verifier_contradiction_penalty_role_aware(
    question: str,
    answer_type: str,
    specific_type: float,
    channels: dict[str, str],
) -> float:
    threshold = float(PATH_FAMILY_ANSWER_VERIFIER_ROLE_AWARE_CONSTANTS["role_aware_specific_threshold"])
    if specific_type >= threshold:
        return 0.0
    q = question.casefold()
    identity_text = f"{channels['candidate_name_text']} {channels['candidate_type_text']}"
    contradictions = [
        (["film series", "franchise"], ["human", "person", "country", "city", "organization"]),
        (["occupation", "profession", "position"], ["human", "person", "film", "book", "city", "country", "company", "organization"]),
        (["legal form", "company type", "business type"], ["human", "person", "film", "city", "country"]),
        (["county"], ["human", "person", "film", "company", "country", "city"]),
        (["city"], ["human", "person", "film", "company", "country", "county"]),
        (["country"], ["human", "person", "film", "company", "city", "county"]),
        (["written work", "book", "novel"], ["human", "person", "city", "country", "company", "event", "war"]),
    ]
    for question_terms, wrong_terms in contradictions:
        if any(term in q for term in question_terms) and any(term in identity_text for term in wrong_terms):
            return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["contradiction_penalty"])
    if answer_type and type_compatibility_text_conflict(answer_type, identity_text):
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["contradiction_penalty"]) * 0.65
    return 0.0


def candidate_evidence_snippets_role_aware(
    graph: KnowledgeGraph,
    entity_id: str,
    ...
    snippets = [
        f"name: {graph.entity_name(entity_id)}",
        *[f"type: {type_name}" for type_name in graph.entity_type_names(entity_id)],
    ]
    snippets.extend(f"relation: {label}" for label in answer_local_relation_labels(graph, entity_id, limit=limit))
    if channels["neighbor_entity_text"]:
        snippets.append(f"neighbors: {channels['neighbor_entity_text'][:160]}")
    if condition_terms:
        snippets.sort(
            key=lambda text: max((char_ngram_similarity(term, text) for term in condition_terms), default=0.0),
            reverse=True,
        )
    return snippets[:limit]


def specific_answer_condition_terms(question: str, answer_type: str) -> list[str]:
    q = question.casefold()
    patterns = [
        (["film series", "movie series", "series of films", "franchise"], ["film series", "franchise", "media franchise", "series", "sequel", "prequel"]),
        (["occupation", "profession", "position"], ["occupation", "profession", "position", "field of work", "job", "office"]),
        (["legal form", "company type", "business type"], ["legal form", "business type", "company type", "corporation", "business entity"]),
        (["county"], ["county"]),
        (["city", "town"], ["city", "town", "municipality"]),
        (["state", "province"], ["state", "province", "administrative territorial entity"]),
        (["country", "sovereign state"], ["country", "sovereign state", "unitary state"]),
        (["written work", "book", "novel", "publication"], ["written work", "book", "novel", "literary work", "publication"]),
        (["university", "school", "institution"], ["university", "school", "educational institution", "institution"]),
        (["human", "person", "who"], ["human", "person"]),
        (["location", "place", "where"], ["location", "place", "city", "country", "state", "province"]),
        (["film", "movie"], ["film", "movie"]),
        (["organization", "company"], ["organization", "company", "enterprise"]),
    ]
    for question_terms, condition_terms in patterns:
        if any(term in q for term in question_terms):
            return condition_terms
    if answer_type:
        return [answer_type.replace("_", " ")]
    return []


def condition_match_score(condition_terms: list[str], evidence_text: str) -> float:
    if not condition_terms:
        return 0.0
    direct_hits = sum(1 for term in condition_terms if term in evidence_text)
    direct = direct_hits / max(1, len(condition_terms))
    fuzzy = max((char_ngram_similarity(term, evidence_text) for term in condition_terms), default=0.0)
    return min(1.0, 0.75 * direct + 0.25 * fuzzy)


def candidate_evidence_snippets(
    graph: KnowledgeGraph,
    entity_id: str,
    condition_terms: list[str],
    ...
    snippets = [f"type: {type_name}" for type_name in graph.entity_type_names(entity_id)]
    snippets.extend(f"relation: {label}" for label in local_relations)
    if condition_terms:
        snippets.sort(
            key=lambda text: max((char_ngram_similarity(term, text) for term in condition_terms), default=0.0),
            reverse=True,
...
def verifier_broad_only_penalty(
    graph: KnowledgeGraph,
    entity_id: str,
    question: str,
    answer_type: str,
    ...
    if asks_specific and broad_score > 0.0:
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"])
    if asks_specific and not local_text:
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["broad_only_penalty"]) * 0.35
    return 0.0


def verifier_contradiction_penalty(question: str, answer_type: str, specific_type: float, local_text: str) -> float:
    if specific_type > 0.0:
        return 0.0
    q = question.casefold()
    contradictions = [
        (["film series", "franchise"], ["human", "person", "country", "city", "organization"]),
        (["occupation", "profession", "position"], ["film", "book", "city", "country", "company", "organization"]),
        (["legal form", "company type", "business type"], ["human", "person", "film", "city", "country"]),
        (["county"], ["human", "person", "film", "company", "country"]),
        (["city"], ["human", "person", "film", "company", "country"]),
        (["country"], ["human", "person", "film", "company", "city"]),
        (["written work", "book", "novel"], ["human", "person", "city", "country", "company"]),
    ]
    for question_terms, wrong_terms in contradictions:
        if any(term in q for term in question_terms) and any(term in local_text for term in wrong_terms):
            return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["contradiction_penalty"])
    if answer_type and type_compatibility_text_conflict(answer_type, local_text):
        return float(PATH_FAMILY_ANSWER_VERIFIER_CONSTANTS["contradiction_penalty"]) * 0.65
    return 0.0


def type_compatibility_text_conflict(answer_type: str, local_text: str) -> bool:
    wrong_patterns = {
        "person": ["film", "book", "city", "country", "organization", "company"],
        "organization": ["human", "person", "city", "country"],
        "occupation": ["human", "person", "film", "city", "country", "company"],
    ...
    return any(term in local_text for term in wrong_patterns.get(answer_type, []))
...
def build_family_answer_ranker_diagnostics(
    ...
    for family_rank, record in enumerate(family_records, start=1):
        ...
        for candidate in candidates:
            ...
            if answer_verifier_role_aware:
                score = role_aware_verifier["verified_final_score"]
                candidate["answer_verifier"] = role_aware_verifier
            elif answer_verifier:
                score = verifier["verified_final_score"]
            ...
            candidate.update({f"verifier_{key}": value for key, value in verifier.items() if isinstance(value, (int, float))})
            candidate.update({f"role_aware_verifier_{key}": value for key, value in role_aware_verifier.items() if isinstance(value, (int, float))})
        ...
        diagnostics.append(
            {
                ...
                "top_answer_before_verification": top.get("answer_label", ""),
                "top_answer_after_verification": top.get("answer_label", ""),
                "gold_rank_after_verification": gold.get("pool_rank"),
            }
        )
    return diagnostics
...
def answer_ranker_diagnostic_candidate(candidate: dict[str, Any]) -> dict[str, Any]:
    if not candidate:
        return {}
    verifier = candidate.get("answer_verifier", {})
    return {
        "answer_id": candidate.get("answer_id", ""),
        "answer_label": candidate.get("answer_label", ""),
        ...
        "verified_final_score": verifier.get("verified_final_score", candidate.get("score", 0.0)) if verifier else candidate.get("score", 0.0),
        "verification_score": verifier.get("verification_score", 0.0) if verifier else candidate.get("verifier_verification_score", 0.0),
        "specific_type_evidence": verifier.get("specific_type_evidence", 0.0) if verifier else candidate.get("verifier_specific_type_evidence", 0.0),
        "remaining_need_evidence": verifier.get("remaining_need_evidence", 0.0) if verifier else candidate.get("verifier_remaining_need_evidence", 0.0),
        "local_relation_evidence": verifier.get("local_relation_evidence", 0.0) if verifier else candidate.get("verifier_local_relation_evidence", 0.0),
        "broad_only_penalty": verifier.get("broad_only_penalty", 0.0) if verifier else candidate.get("verifier_broad_only_penalty", 0.0),
        "contradiction_penalty": verifier.get("contradiction_penalty", 0.0) if verifier else candidate.get("verifier_contradiction_penalty", 0.0),
        "evidence_snippets": verifier.get("evidence_snippets", []) if verifier else [],
    }
...
def path_family_search_result(
    ...
    for state in states:
        for entity_id in state.frontier_ids:
            ...
            row["raw_support_count"] += 1
            row["best_path_score"] = max(row["best_path_score"], final_score)
            row["paths"].append(
                {
    ...
    for candidate in candidates:
        ...
        if minimal_ranker:
            ...
            candidate["answer_verifier"] = verifier
            candidate["old_path_family_final_score"] = verifier["old_path_family_final_score"]
            candidate["verified_final_score"] = verifier["verified_final_score"]
        else:
...
def build_method_regression_audit(
    rows: list[dict[str, Any]],
    baseline_key: str,
    candidate_key: str,
    title: str,
    ...
    for row in rows:
        ...
        baseline_hit = bool(baseline["hits_at_1"])
        candidate_hit = bool(candidate["hits_at_1"])
        if candidate_hit and not baseline_hit:
            label = "candidate_correct_baseline_wrong"
        elif baseline_hit and not candidate_hit:
            label = "baseline_correct_candidate_wrong"
        elif baseline_hit:
            label = "both_correct"
        else:
            label = "both_wrong"
        baseline_top = baseline.get("top_answer") or {}
        candidate_top = candidate.get("top_answer") or {}
        cases.append(
            {
                "question_id": row["question_id"],
                "question": row["question"],
                ...
                "label": label,
                "baseline_top_answer": baseline_top.get("answer_label", ""),
                "candidate_top_answer": candidate_top.get("answer_label", ""),
                "baseline_gold_generated": bool(baseline.get("gold_generated", False)),
                "candidate_gold_generated": bool(candidate.get("gold_generated", False)),
            }
        )
    ...
    return {
        ...
        "summary": {
            ...
            "candidate_correct_baseline_wrong": counts["candidate_correct_baseline_wrong"],
            "baseline_correct_candidate_wrong": counts["baseline_correct_candidate_wrong"],
            "baseline_hits_at_1": counts["both_correct"] + counts["baseline_correct_candidate_wrong"],
            "candidate_hits_at_1": counts["both_correct"] + counts["candidate_correct_baseline_wrong"],
            "baseline_gold_generated": sum(1 for case in cases if case["baseline_gold_generated"]),
            "candidate_gold_generated": sum(1 for case in cases if case["candidate_gold_generated"]),
            "total_questions": len(cases),
        },
        "cases": cases,
    }
...
def write_method_regression_markdown(audit: dict[str, Any]) -> str:
    lines = [
        f"# {audit['title']}",
        "",
        f"Baseline: `{audit['baseline_key']}`  Candidate: `{audit['candidate_key']}`",
        "",
        audit["note"],
        "",
        "```json",
    ...
    for label in ["candidate_correct_baseline_wrong", "baseline_correct_candidate_wrong", "both_wrong"]:
        ...
        for case in selected:
            ...
            lines.append(
                f"  - baseline top: `{case['baseline_top_answer']}` candidate top: `{case['candidate_top_answer']}`"
            )
            lines.append(
                f"  - gold_generated baseline/candidate: {case['baseline_gold_generated']}/{case['candidate_gold_generated']}"
            )
        lines.append("")
    return "\n".join(lines)
...
def print_runtime_log(row: dict[str, Any], index: int, total: int) -> None:
    ...
    path_family_ah_adj = row["path_family_any_hop_adjudicated"]
    future = row["future_aware_proof_state_beam"]
    future_v2 = row["future_aware_v2_proof_state_beam"]
    baseline_top = baseline["top_answer"] or {}
    proof_top = proof["top_answer"] or {}
    ...
    path_family_ah_adj_top = path_family_ah_adj["top_answer"] or {}
    future_top = future["top_answer"] or {}
    future_v2_top = future_v2["top_answer"] or {}
    ...
    print(f"Path-family any-hop adjudicated top answer: {path_family_ah_adj_top.get('answer_label', '<none>')}", flush=True)
    ...
    print(f"Gold generated path-family any-hop adjudicated: {'yes' if path_family_ah_adj['gold_generated'] else 'no'}", flush=True)
    ...
    print(f"Path-family LLM retention evidence: {top_path_readable(path_family_llm_ret_top)}", flush=True)
    print(f"Path-family wide symbolic evidence: {top_path_readable(path_family_wide_sym_top)}", flush=True)
